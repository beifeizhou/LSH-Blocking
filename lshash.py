...
class LSHash(object):
	...
	def _generate_uniform_planes(self):
		...
		sb = np.random.randn(self.hash_size, self.input_dim)
		return sb
	...
	def query(self, query_point, num_results=None, distance_func=None):
		...
		print '--- CANDICATE NUMBER: ' + str(len(candidates)) + ' ---' #yuanhao
	...
	def jaccard_dist(x, y):
		assert len(x) == len(y), "error: jaccard operands not compatible"
		count = 0
